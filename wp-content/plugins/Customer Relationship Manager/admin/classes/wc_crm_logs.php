                  ...
                  <td>
                      <?php
                        ...
                        if(!empty($user)){
                          ...
                          $orders_data = $wpdb->get_results("SELECT first_name, last_name FROM $table_name WHERE email = '$email'");
                          $order = $orders_data[0];
              ...
              <tr>
                ...
                <td>
                  <?php
                    ...
                    global $wpdb;
                    $sql = "SELECT * FROM {$wpdb->prefix}wc_crm_customers                    
                    ...
                    foreach ($results as $customer) {
                      if($d) echo ', ';
                      if( !empty($customer->user_id) && $customer->user_id != NULL)
                      echo '<a href="admin.php?page=wc-customer-relationship-manager&action=email&user_id='.$customer->user_id.'">'.$customer->email.'</a> (<a href="admin.php?page=wc_new_customer&user_id='.$customer->user_id.'">'.$customer->first_name.' '.$customer->last_name.'</a>)';
                      elseif( !empty($customer->order_id) && $customer->order_id != NULL)
                      echo '<a href="admin.php?page=wc-customer-relationship-manager&action=email&order_id='.$customer->order_id.'">'.$customer->email.'</a> (<a href="admin.php?page=wc_new_customer&order_id='.$customer->order_id.'">'.$customer->first_name.' '.$customer->last_name.'</a>)';
                      $d++;
                    }
                   ?>
                </td>
