...
/**
		...
		global $thepostid, $post, $woocommerce;
			...
			<div class="panel-wrap woocommerce" id="customer_data">
				<div id="order_data" class="panel">
                    ...
                    if ( empty($selected) ) $selected ='Customer';
						...
						<div class="order_data_column" id="order_data_column_billing">
							...
							<?php
								...
								foreach ( $this->billing_fields as $key => $field ) {
									...
									switch ( $field['type'] ) {
										case "select" :
											woocommerce_wp_select( array( 'id' => '_billing_' . $key, 'class' => implode(' ', $field['class']), 'label' => $field['label'], 'options' => $field['options'], 'value' => $value ) );
										break;
										default :
											woocommerce_wp_text_input( array( 'id' => '_billing_' . $key, 'class' => implode(' ', $field['class']), 'label' => $field['label'], 'value' => $value ) );
										break;
									}
								}
					...
					<?php $address_l = str_replace('<br/>', ', ',$this->get_formatted_billing_address( true )) ; ?>
	...
	function create_user() {
      	...
      	$this->save($user_id, true);
				update_user_meta( $user_id, 'nickname', str_replace(' ', '', ucfirst(strtolower($_POST['first_name'])) ) . str_replace(' ', '', ucfirst(strtolower($_POST['last_name'])) ) );

				wp_update_user( array( 'ID' => $user_id, 'role' => 'customer' ) );
			} else {
				$this->error[] = __('<p><strong>ERROR</strong>: User already exists.</p>');
			}
	}
