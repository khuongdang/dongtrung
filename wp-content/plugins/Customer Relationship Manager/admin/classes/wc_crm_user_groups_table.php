...
class WC_Pos_Groups_Table extends WP_List_Table {
  ...
  function column_group_action( $item ) {
    return sprintf(
      '<a href="admin.php?page=wc_user_grps&action=view&id=%s" class="button tips view" data-tip="' . esc_attr__( 'View Customers in Group', 'wc_customer_relationship_manager' ) . '">'.esc_attr__( 'View Customers in Group', 'wc_customer_relationship_manager' ).'</a>', $item['ID']
    );
  }
  function column_group_terms( $item ) {
    if($item['group_type'] == 'dynamic'){
      ...
      if(!empty($item['group_customer_status'])) $output .= 'Customer status is ' . $item['group_customer_status'] . '<br />';
  ...
  function column_group_name( $item ) {
    ...
    $name = sprintf(
        '<strong><a style="display: block;" href="admin.php?page=%s&action=%s&id=%s">%s</a></strong>', 'wc_user_grps', 'view', $item['ID'], $item['group_name']
    );
