...
class WC_Crm_Customer_Groups {
	...
	public function output() {
		...
		if ( 'add' === $action || 'edit' === $action ) {
			...
			$group_customer_status  = ( isset( $_POST['group_customer_status'] ) )    ? (string) stripslashes( $_POST['group_customer_status'] ) : '';
			...
			if ( ! empty( $error ) ) {
				...
				if ( 'add' === $action ) {
					...
					$group = array(
						...
						'group_customer_status' => $group_customer_status,
		...
		if ( 'delete' === $action ) {
			...
			do_action( 'wc_crm_group_deleted', $group_id, $group_name, $taxonomy );
	...
	public function edit_group() {
		...
		$group_customer_status  = ( isset( $_POST['group_customer_status'] ) )    ? (string) stripslashes( $_POST['group_customer_status'] ) : $group_to_edit->group_customer_status;
				...
				<table class="form-table">
					<tbody>
								...
								<tr class="form-field dynamic_group_type">
									...
									<td>
										<select name="group_customer_status" id="group_customer_status">
											<option value="any">
						            	<?php _e( 'Any', 'wc_customer_relationship_manager' ); ?>
					            </option>
											<?php
						          ...
						          foreach ( $statuses as $status => $name ) : ?>
						            <option value="<?php echo strtolower($name); ?>" <?php selected( $group_customer_status, strtolower($name) ); ?>>
						            	<?php _e( $name, 'wc_customer_relationship_manager' ); ?>
						            </option>
						          <?php
						          endforeach;
	...
	public function view_customers_in_group() {
		require_once( 'classes/wc_crm_customers_table.php' );
		$wc_crm_customers_table = new WC_Crm_Customers_Table();
		?>
		<div class="wrap" id="wc-crm-page"><div class="icon32"></div>
		<h2><?php __( 'Orders', 'wc_customer_relationship_manager' ) ?></h2>
