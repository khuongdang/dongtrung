...
if ( ! class_exists( 'YITH_WCAF_Shortcode' ) ) {
	...
	class YITH_WCAF_Shortcode {
		...
		public static function affiliate_dashboard_settings( $atts = array() ){
			...
			$notify_pending_commissions = isset( $user->_yith_wcaf_notify_pending_commission ) ? $user->_yith_wcaf_notify_pending_commission : 'no';
			$notify_paid_commissions = isset( $user->_yith_wcaf_notify_paid_commission ) ? $user->_yith_wcaf_notify_paid_commission : 'no';
			...
			if( $change ){
				wc_add_notice( __( 'Changes correctly saved!', 'yith-wcaf' ) );
			}
		...
		public static function affiliate_dashboard_summary( $atts = array() ){
			...
			$greeting_message = sprintf(
				__( 'Hello <strong>%1$s</strong> (not %1$s? <a href="%2$s">Sign out</a>).', 'yith-wcaf' ) . ' ',
				$user->display_name,
