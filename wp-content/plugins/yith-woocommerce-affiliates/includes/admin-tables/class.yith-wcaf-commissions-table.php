	...
	class YITH_WCAF_Commissions_Table extends WP_List_Table {
		...
		public function column_order( $item ) {
			...
			if ( $address = $order->get_formatted_billing_address() ) {
				$customer_tip[] = __( 'Billing:', 'yith-wcaf' ) . ' ' . $address . '<br/><br/>';
			}

			if ( $order->billing_phone ) {
				$customer_tip[] = __( 'Tel:', 'yith-wcaf' ) . ' ' . $order->billing_phone;
			}
		...
		public function get_columns(){
			$columns = array(
				...
				'id'                    => __( 'ID', 'yith-wcaf' ),
				'status'                => sprintf( '<span class="status_head tips" data-tip="%s">%s</span>', __( 'Status', 'yith-wcaf' ), __( 'Status', 'yith-wcaf' ) ),
				'order'                 => __( 'Order', 'yith-wcaf' ),
				'user'                  => __( 'User', 'yith-wcaf' ),
				'product'               => __( 'Product', 'yith-wcaf' ),
				'line_item_total'       => __( 'Total', 'yith-wcaf' ),
				'rate'                  => __( 'Rate', 'yith-wcaf' ),
				'amount'                => __( 'Commission', 'yith-wcaf' ),
				'payments'              => __( 'Payment', 'yith-wcaf' ),
				'date'                  => __( 'Date', 'yith-wcaf' )
			);
		...
		public function get_views() {
			...
			return array(
				'all' => sprintf( '<a href="%s" class="%s">%s <span class="count">(%s)</span></a>', add_query_arg( 'status', 'all' ), $current == 'all' ? 'current' : '', __( 'All', 'yith-wcaf' ), YITH_WCAF_Commission_Handler()->per_status_count( 'all' )  ),
				'pending' => sprintf( '<a href="%s" class="%s">%s <span class="count">(%s)</span></a>', add_query_arg( 'status', 'pending' ), $current == 'pending' ? 'current' : '', __( 'Pending', 'yith-wcaf' ), YITH_WCAF_Commission_Handler()->per_status_count( 'pending' )  ),
				'pending-payment' => sprintf( '<a href="%s" class="%s">%s <span class="count">(%s)</span></a>', add_query_arg( 'status', 'pending-payment' ), $current == 'pending-payment' ? 'current' : '', __( 'Pending Payment', 'yith-wcaf' ), YITH_WCAF_Commission_Handler()->per_status_count( 'pending-payment' )  ),
				'paid' => sprintf( '<a href="%s" class="%s">%s <span class="count">(%s)</span></a>', add_query_arg( 'status', 'paid' ), $current == 'paid' ? 'current' : '', __( 'Paid', 'yith-wcaf' ), YITH_WCAF_Commission_Handler()->per_status_count( 'paid' )  ),
				'not-confirmed' => sprintf( '<a href="%s" class="%s">%s <span class="count">(%s)</span></a>', add_query_arg( 'status', 'not-confirmed' ), $current == 'not-confirmed' ? 'current' : '', __( 'Not Confirmed', 'yith-wcaf' ), YITH_WCAF_Commission_Handler()->per_status_count( 'not-confirmed' )  ),
				'cancelled' => sprintf( '<a href="%s" class="%s">%s <span class="count">(%s)</span></a>', add_query_arg( 'status', 'cancelled' ), $current == 'cancelled' ? 'current' : '', __( 'Cancelled', 'yith-wcaf' ), YITH_WCAF_Commission_Handler()->per_status_count( 'cancelled' )  ),
				'refunded' => sprintf( '<a href="%s" class="%s">%s <span class="count">(%s)</span></a>', add_query_arg( 'status', 'refunded' ), $current == 'refunded' ? 'current' : '', __( 'Refunded', 'yith-wcaf' ), YITH_WCAF_Commission_Handler()->per_status_count( 'refunded' )  ),
			);
		}
		...
		public function get_bulk_actions() {
			$actions = array(
				'switch-to-pending'         => __( 'Switch to pending', 'yith-wcaf' ),
				'switch-to-not-confirmed'   => __( 'Switch to not confirmed', 'yith-wcaf' ),
				'switch-to-cancelled'       => __( 'Switch to cancelled', 'yith-wcaf' ),
				'switch-to-refunded'        => __( 'Switch to refunded', 'yith-wcaf' ),
				'pay'                       => __( 'Create a payment', 'yith-wcaf' )
			);
		...
		protected function extra_tablenav( $which ) {
			...
			?>
			<input style="min-width: 200px; vertical-align:middle;" class="wc-product-search" type="hidden" name="_product_id" value="<?php echo esc_attr( $product_id ) ?>" data-placeholder="<?php _e( 'Select a product', 'yith-wcaf' ) ?>" data-selected="<?php echo esc_attr( $selected_product ) ?>" />
			<input style="min-width: 200px; vertical-align:middle;" class="wc-customer-search" type="hidden" name="_user_id" value="<?php echo esc_attr( $user_id ) ?>" data-placeholder="<?php _e( 'Select a user', 'yith-wcaf' ) ?>" data-selected="<?php echo esc_attr( $selected_user ) ?>" />
			<input style="min-width: 200px; vertical-align:middle;" class="date-picker" type="text" name="_from" value="<?php echo esc_attr( $from ) ?>" placeholder="<?php _e( 'From:', 'yith-wcaf' )?>" />
			<input style="min-width: 200px; vertical-align:middle;" class="date-picker" type="text" name="_to" value="<?php echo esc_attr( $to ) ?>" placeholder="<?php _e( 'To:', 'yith-wcaf' )?>" />
			<?php
			submit_button( __( 'Filter', 'yith-wcaf' ), 'button', 'filter_action', false, array( 'id' => 'post-query-submit' ) );

			if( $need_reset ) {
				echo sprintf( '<a href="%s" class="button">%s</a>', esc_url( add_query_arg( array(
					'page' => 'yith_wcaf_panel',
					'tab'  => 'commissions'
				), admin_url( 'admin.php' ) ) ), __( 'Reset', 'yith-wcaf' ) );
			}
		}
