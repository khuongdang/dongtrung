...
if ( ! class_exists( 'YITH_WCAF_Product_Stat_Table' ) ) {
	...
	class YITH_WCAF_Product_Stat_Table extends WP_List_Table {
		...
		public function column_product( $item ) {
			...
			$actions = array(
				'ID' => $item['ID'],
				'view' => sprintf( '<a href="%s">%s</a>', get_the_permalink( $item['ID'] ), __( 'View', 'yith-wcaf' ) ),
				'view_commissions' => sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( array( 'page' => 'yith_wcaf_panel', 'tab' => 'commissions', '_product_id' => $item['ID'] ), admin_url( 'admin.php' ) ) ), __( 'View commissions', 'yith-wcaf' ) )
			);

			$column .= sprintf( '<a href="%s">%s</a>%s', get_edit_post_link( $product->id ), $product->get_title(), $this->row_actions( $actions ) );
			return $column;
		}
		...
		public function column_clicks( $item ) {
			...
			$column = ! empty( $item['clicks'] ) ? $item['clicks'] : __( 'N/A', 'yith-wcaf' );
			return $column;
		}
		...
		public function column_conversions( $item ) {
			...
			$column = ! empty( $item['conversions'] ) ? $item['conversions'] : __( 'N/A', 'yith-wcaf' );
			return $column;
		}
		...
		public function column_conversion_time( $item ) {
			...
			$column = ! empty( $item['conversion_time'] ) ? human_time_diff( time(), time() + $item['conversion_time'] ) : __( 'N/A', 'yith-wcaf' );
			return $column;
		}
		...
		public function get_columns(){
			$columns = array(
				'thumb'             => sprintf( '<span class="wc-image" data-tip="%s"></span>', __( 'Image', 'yith-wcaf' ) ),
				'product'           => __( 'Product', 'yith-wcaf' ),
				'sales'             => __( 'Commissions', 'yith-wcaf' ),
				'refunds'           => __( 'Refunds', 'yith-wcaf' ),
				'clicks'            => __( 'Clicks', 'yith-wcaf' ),
				'conversions'       => sprintf( '<span class="tips" data-tip="%s">%s</span>', __( 'Number of products sold following a click', 'yith-wcaf' ),  __( 'Conversions', 'yith-wcaf' ) ),
				'conversion_time'   => __( 'Conv. time', 'yith-wcaf' )
			);
		...
		public function prepare_items() {
			...
			$from = isset( $_REQUEST['_from'] ) ? date( 'Y-m-d 00:00:00', strtotime( trim( $_REQUEST['_from'] ) ) ) : false;
			$to = isset( $_REQUEST['_to'] ) ? date( 'Y-m-d 23:59:59', strtotime( trim( $_REQUEST['_to'] ) ) ) : false;
