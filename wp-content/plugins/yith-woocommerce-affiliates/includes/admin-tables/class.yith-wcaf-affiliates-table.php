...
if ( ! class_exists( 'YITH_WCAF_Affiliates_Table' ) ) {
	...
	class YITH_WCAF_Affiliates_Table extends WP_List_Table {
		...
		public function column_status( $item ) {
			...
			$human_friendly_status = ( $enabled ) ? __( 'Enabled', 'yith-wcaf' ) : __( 'Disabled', 'yith-wcaf' );
			$column = sprintf( '<mark class="%s tips" data-tip="%s">%s</mark>', ( $enabled ) ? 'enabled' : 'disabled', $human_friendly_status, $human_friendly_status );
		...
		public function column_affiliate ( $item ) {
			...
			$column .= sprintf( '%s%s<small class="meta email">%s</small><small class="meta">%s: %s</small>', get_avatar( $item['user_id'], 32 ), $username, $user_email, __( 'Payment', 'yith-wcaf' ), $payment_email );
		...
		public function get_views() {
			...
			$tab_url = add_query_arg( array( 'page' => 'yith_wcaf_panel', 'tab' => 'affiliates' ), admin_url( 'admin.php' ) );

			return array(
				'all' => sprintf( '<a href="%s" class="%s">%s <span class="count">(%s)</span></a>', esc_url( add_query_arg( 'status', 'all', $tab_url ) ), $current == 'all' ? 'current' : '', __( 'All', 'yith-wcaf' ), YITH_WCAF_Affiliate_Handler()->per_status_count( 'all' )  ),
				'enabled' => sprintf( '<a href="%s" class="%s">%s <span class="count">(%s)</span></a>', esc_url( add_query_arg( 'status', 'enabled', $tab_url ) ), $current == 'enabled' ? 'current' : '', __( 'Enabled', 'yith-wcaf' ), YITH_WCAF_Affiliate_Handler()->per_status_count( 'enabled' )  ),
				'disabled' => sprintf( '<a href="%s" class="%s">%s <span class="count">(%s)</span></a>', esc_url( add_query_arg( 'status', 'disabled', $tab_url ) ), $current == 'disabled' ? 'current' : '', __( 'Disabled', 'yith-wcaf' ), YITH_WCAF_Affiliate_Handler()->per_status_count( 'disabled' )  )
			);
		}
		...
		public function get_bulk_actions() {
			$actions = array(
				'delete' => __( 'Delete', 'yith-wcaf' ),
				'enable' => __( 'Enable', 'yith-wcaf' ),
				'disable' => __( 'Disabled', 'yith-wcaf' )
			);
			return $actions;
		}
		...
		public function process_bulk_actions() {
			if( ! empty( $_REQUEST[ 'affiliates' ] ) ) {
				...
				switch ( $current_action ) {
					...
					case 'disable':
						foreach( $_REQUEST[ 'affiliates' ] as $affiliate_id ){
							YITH_WCAF_Affiliate_Handler()->update( $affiliate_id, array( 'enabled' => 0 ) );
						}
						break;
				}
			}
