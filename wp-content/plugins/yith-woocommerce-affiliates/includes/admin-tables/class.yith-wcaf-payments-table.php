	...
	class YITH_WCAF_Payments_Table extends WP_List_Table {
		...
		public function column_commissions( $item ) {
			...
			if( empty( $commissions ) ){
				...
				foreach( $commissions as $commission ){
					$column .= $first ? '' : ' | ';
					$column .= sprintf( '<a href="%s">%d</a>', esc_url( add_query_arg( array( 'page' => 'yith_wcaf_panel', 'tab' => 'commissions', 'commission_id' => $commission['ID'] ), admin_url( 'admin.php' ) ) ), $commission['ID'] );
		...
		public function get_columns(){
			$columns = array(
				...
				'payment_email'         => __( 'Payment email', 'yith-wcaf' ),
				'amount'                => __( 'Amount', 'yith-wcaf' ),
				'created_at'            => __( 'Created at', 'yith-wcaf' )
			);
		...
		public function get_bulk_actions() {
			$actions = array(
				'switch-to-completed' => __( 'Switch to completed', 'yith-wcaf' ),
				'switch-to-on-hold' => __( 'Switch to on hold', 'yith-wcaf' )
			);
		...
		protected function extra_tablenav( $which ) {
			...
			?>
			<input style="min-width: 200px; vertical-align:middle;" class="yith-users-select wc-product-search" type="hidden" name="_affiliate_id" data-action="json_search_affiliates" value="<?php echo esc_attr( $user_id ) ?>" data-placeholder="<?php _e( 'Select an affiliate', 'yith-wcaf' ) ?>" data-selected="<?php echo esc_attr( $selected_user )?>" />
			...
			if( $need_reset ){
				echo sprintf( '<a href="%s" class="button">%s</a>', esc_url( add_query_arg( array( 'page' => 'yith_wcaf_panel', 'tab' => 'payments' ), admin_url( 'admin.php' ) ) ) , __( 'Reset', 'yith-wcaf' ) );
			}
		}
