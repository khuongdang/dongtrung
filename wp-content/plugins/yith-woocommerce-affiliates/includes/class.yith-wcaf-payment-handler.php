	...
	class YITH_WCAF_Payment_Handler {
		...
		public function __construct() {
			...
			$this->_status_labels_map = apply_filters( 'yith_wcaf_payment_status_labels_map', array(
				...
				'completed' => __( 'Completed', 'yith-wcaf' ),
				'cancelled' => __( 'Cancelled', 'yith-wcaf' ),
				'on-hold' => __( 'On Hold', 'yith-wcaf' )
			) );
		...
		public function delete( $payment_id ) {
			...
			return $wpdb->delete( $wpdb->yith_payment_commission, array( 'ID' => $payment_id ) );
		}
		 ...
		 * @since 1.0.0
		 */
		public function get_commission_notes( $payment_id ) {
			global $wpdb;
		...
		public function ajax_add_note(){
			...
			$note_date = current_time( 'mysql' );
			$template = '';
			$res = $this->add_note( array( 'payment_id' => $payment_id, 'note_content' => $note_content, 'note_date' => $note_date ) );
		...
		public function get_payments( $args = array() ) {
			...
			if ( ! empty( $args['completed_between'] ) && is_array( $args['completed_between'] ) && ( isset( $args['completed_between']['start_date'] ) || isset( $args['completed_between']['end_date'] ) ) ) {
				if ( ! empty( $args['completed_between']['start_date'] ) ) {
					...
					$query_arg[] = $args['completed_between']['start_date'];
				}

				if ( ! empty( $args['completed_between']['end_date'] ) ) {
					$query .= ' AND yp.completed_at <= %s';
					$query_arg[] = $args['completed_between']['end_date'];
				}
			}
		...
		public function get_commission_payments( $commission_id, $status = 'all' ) {
			...
			if ( ! empty( $payments_ids ) ) {
				foreach ( $payments_ids as $id ) {
					...
					if( ! empty( $res ) ){
						if( $status == 'active' && ! in_array( $res['status'], array( 'on-hold', 'pending', 'completed' ) ) ){
							continue;
						}
		...
		public function change_payment_status( $payment_id, $new_status ) {
			...
			return $this->update( $payment_id, array( 'status' => $new_status ) );
		}
			...
			if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'payments' ) {
				$columns = array_merge(
					...
					array(
						...
						'completed_at'      => __( 'Completed at', 'yith-wcaf' ),
					)
				);
			}
		...
		public static function get_instance(){
			if( class_exists( 'YITH_WCAF_Payment_Handler_Premium' ) ) {
				if ( is_null( YITH_WCAF_Payment_Handler_Premium::$instance ) ) {
					YITH_WCAF_Payment_Handler_Premium::$instance = new YITH_WCAF_Payment_Handler_Premium;
				}

				return YITH_WCAF_Payment_Handler_Premium::$instance;
			}
			else{
