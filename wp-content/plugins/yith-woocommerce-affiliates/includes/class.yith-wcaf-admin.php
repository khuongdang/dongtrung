	...
	class YITH_WCAF_Admin {
		...
		public function __construct() {
			...
			$this->available_tabs = apply_filters( 'yith_wcaf_available_admin_tabs', array(
				'commissions' => __( 'Commissions', 'yith-wcaf' ),
				'affiliates' => __( 'Affiliates', 'yith-wcaf' ),
		...
		public function register_panel() {
			$args = array(
				...
				'parent_slug'   => '',
				'page_title'    => __( 'Affiliates', 'yith-wcaf' ),
				'menu_title'    => __( 'Affiliates', 'yith-wcaf' ),
		...
		public function enqueue() {
			...
			if( in_array( $screen->id, $this->get_screen_ids() ) ) {
				...
				wp_localize_script( 'yith-wcaf', 'yith_wcaf', array(
					'empty_row' => sprintf( '<tr class="no-items"><td class="colspanchange" colspan="5">%s</td></tr>', __( 'No items found.', 'yith-wcaf' ) ),
					'tabs_badges' => array(
			...
			if( in_array( $screen->id, $this->get_screen_ids() ) ){
				...
				if( isset( $_GET['commission_status_change'] ) ){
					if( $_GET['commission_status_change'] == 1 ){
						$messages['success'][] = __( 'Status changed correctly', 'yith-wcaf' );
					}
					else{
						$messages['notice'][] = __( 'An error occurred with status changing', 'yith-wcaf' );
					}
				}

				if( isset( $_GET['commission_payment_failed'] ) ){
					$errors = explode( ',', $_GET['commission_payment_failed'] );
					$messages['notice'][] = sprintf( __( 'An error occurred during payment processing: %s', 'yith-wcaf' ), implode( ', ', $errors ) );
				}

				if( ! empty( $_GET['commission_paid'] ) ){
					$paid_commissions = explode( ',', $_GET['commission_paid'] );
					$messages['success'][] = sprintf( __( 'Commissions %s correctly paid', 'yith-wcaf' ), implode( ', ', $paid_commissions ) );
				}

				if( ! empty( $_GET['commission_unpaid'] ) ){
					$unpaid_commissions = explode( ',', $_GET['commission_unpaid'] );
					$messages['notice'][] = sprintf( __( 'Commissions %s could not be paid', 'yith-wcaf' ), implode( ', ', $unpaid_commissions ) );
				}
		...
		public function action_links( $links ) {
			$plugin_links = array(
				'<a href="' . admin_url( 'admin.php?page=yith_wcaf_panel&tab=settings' ) . '">' . __( 'Settings', 'yith-wcaf' ) . '</a>'
			);
		...
		public function print_stat_panel() {
			$page_title = __( 'Global stats', 'yith-wcaf' );
			$title_suffix = '';
			...
			if( ! empty( $from ) || ! empty( $to ) ){
				$title_suffix = sprintf( ' (%s - %s)', ! empty( $from ) ? date_i18n( wc_date_format(), strtotime( $from ) ) : __( 'Ever', 'yith-wcaf' ), ! empty( $to ) ? date_i18n( wc_date_format(), strtotime( $to ) ) : __( 'Today', 'yith-wcaf' ) );
			}
			...
			$avg_conv_rate = ! empty( $avg_conv_rate ) ? number_format( $avg_conv_rate, 2 ) . '%' : __( 'N/A', 'yith-wcaf' );
