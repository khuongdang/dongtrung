	...
	class YITH_WCAF_Commission_Handler {
		...
		public function __construct() {
			...
			$this->_status_labels_map = apply_filters( 'yith_wcaf_commission_status_labels_map', array(
				'pending' => __( 'Pending', 'yith-wcaf' ),
				'pending-payment' => __( 'Pending Payment', 'yith-wcaf' ),
				'paid' => __( 'Paid', 'yith-wcaf' ),
				'not-confirmed' => __( 'Not confirmed', 'yith-wcaf' ),
				'cancelled' => __( 'Cancelled', 'yith-wcaf' ),
				'refunded' => __( 'Refunded', 'yith-wcaf' )
			) );
		...
		public function create_order_commissions( $order_id, $token, $token_origin = 'undefined' ){
			...
			update_post_meta( $order_id, '_yith_wcaf_referral', $token );
			...
			if( ! empty( $items ) ){
				foreach( $items as $item_id => $item ){
					...
					if( ! $rate ) {
						$rate = YITH_WCAF_Rate_Handler()->get_rate( $affiliate, intval( $product_id ) );
						...
						if ( $this->_persistent_calculation == 'yes' && $token_origin == 'persistent' ) {
							$rate = YITH_WCAF_Rate_Handler()->get_persistent_rate( $rate );
						}
					}

					$commission = $this->_calculate_line_item_commission( $order, $item, $rate );
		 ...
		 * @since 1.0.0
		 */
		public function regenerate_order_commissions( $order_id ){
			$order = wc_get_order( $order_id );
			$token = get_post_meta( $order_id, '_yith_wcaf_referral', true );
			...
			$res = $this->delete_order_commissions( $order_id );
		 ...
		 * @since 1.0.0
		 */
		public function delete_order_commissions( $order_id ) {
			$order   = wc_get_order( $order_id );
			$token = get_post_meta( $order_id, '_yith_wcaf_referral', true );
			$affiliate = YITH_WCAF_Affiliate_Handler()->get_affiliate_by_token( $token );
			...
			if ( ! empty( $order_commissions ) ) {
				...
				foreach ( $order_commissions as $commission ) {
					YITH_WCAF_Affiliate_Handler()->update_affiliate_total( $affiliate['ID'], - 1 * (double) $commission['amount'] );
		...
		public function order_trashing_handler( $post_id ) {
			...
			$this->delete_order_commissions( $post_id );
		}
		...
		public function order_untrashing_handler( $post_id ){
			$order = wc_get_order( $post_id );
			$token = get_post_meta( $post_id, '_yith_wcaf_referral', true );
		...
		public function order_refund_handler( $new_refund_id ) {
			...
			foreach ( $order->get_refunds() as $_refund ) {
				$refunded_commissions = get_post_meta( $_refund->id, '_refunded_commissions', true );
				...
				if( ! empty( $refunded_commissions ) ) {
					...
					update_post_meta( $_refund->id, '_refunded_commissions', $refunded_commissions );
				}
			}
			...
			foreach ( $order->get_refunds() as $_refund ) {
				...
				foreach ( $_refund->get_items() as $item_id => $item ) {
					...
					if ( $commission_id = $order->get_item_meta( $original_item_id, '_yith_wcaf_commission_id', true ) ) {
			...
			foreach ( $global_refunds as $_refund ) {
				...
				foreach ( $order->get_items() as $item_id => $item ) {
					$commission_id = $order->get_item_meta( $item_id, '_yith_wcaf_commission_id', true );
					if ( $commission_id ) {
						$commission = $this->get_commission( $commission_id );
			...
			foreach ( $refunds as $refund_id => $commissions_refunded ) {
				foreach ( $commissions_refunded as $commission_id => $amount ) {
					$commission = $this->get_commission( $commission_id );
					$note = $refund_id == $new_refund_id ? sprintf( __( 'Refunded %s due to refund #%s creation', 'yith-wcaf' ), wc_price( abs( $amount ) ), $refund_id ) : '';
			...
			if( $meta_key == '_refunded_commissions' ){
				...
				if( ! empty( $meta_value ) ){
					foreach( $meta_value as $commission_id => $refund ){
						$commission = $this->get_commission( $commission_id );
						$this->change_commission_amount( $commission_id, abs( (double) $refund ), sprintf( __( 'Restored %s due to refund #%s deletion', 'yith-wcaf' ), wc_price( abs( $refund ) ), $object_id ) );
		...
		public function delete( $commission_id ) {
			...
			return $wpdb->delete( $wpdb->yith_commissions, array( 'ID' => $commission_id ) );
		}
		 ...
		 * @since 1.0.0
		 */
		protected function _calculate_line_item_commission( $order, $line_item, $rate ){
		...
		public function ajax_add_note(){
			...
			$note_date = current_time( 'mysql' );
			$template = '';
			$res = $this->add_note( array( 'commission_id' => $commission_id, 'note_content' => $note_content, 'note_date' => $note_date ) );
			...
			if( $res ){
				$template = sprintf( '<li rel="%s" class="note">
					...
					$note_content,
					$note_date,
					sprintf( __( 'added on %1$s at %2$s', 'yith-wcaf' ), date_i18n( wc_date_format(), strtotime( $note_date ) ), date_i18n( wc_time_format(), strtotime( $note_date ) ) ),
					__( 'Delete note', 'yith-wcaf' )
				);
			}
		...
		public function get_commissions( $args = array() ) {
			...
			if( ! empty( $args['status'] ) && in_array( $args['status'], $this->_available_commission_status ) ){
			...
			$query .= ' GROUP BY yc.line_item_id';
				...
				if( $order ){
					...
					if( $refunds_objects ){
						foreach( $refunds_objects as $refund ){
							if( $refund->id ) {
								$refunded_commissions = get_post_meta( $refund->id, '_refunded_commissions', true );

								if ( isset( $refunded_commissions[ $commission_id ] ) ) {
									$refunds[ $refund->id ] = $refunded_commissions[ $commission_id ];
								}
							}
						}
					}
		...
		public function add_screen_option() {
			if ( 'yith-plugins_page_yith_wcaf_panel' == get_current_screen()->id && ( ! isset( $_GET['tab'] ) || ( isset( $_GET['tab'] ) && $_GET['tab'] == 'commissions' ) ) ) {
				add_screen_option( 'per_page', array( 'label' => __( 'Commissions', 'yith-wcaf' ), 'default' => 20, 'option' => 'edit_commissions_per_page' ) );
		...
		public function add_order_metabox() {
			add_meta_box( 'yith_wcaf_order_referral_commissions', __( 'Referral Commissions', 'yith-wcaf' ), array( $this, 'print_referral_commissions_metabox' ), 'shop_order', 'side' );
		}
		...
		public function print_referral_commissions_metabox( $post ) {
			...
			$token = get_post_meta( $post->ID, '_yith_wcaf_referral', true );
			$affiliate = YITH_WCAF_Affiliate_Handler()->get_affiliate_by_token( $token );
			...
			if( $affiliate ){
				...
				if( $referral ){
					...
					$commissions = YITH_WCAF_Commission_Handler()->get_commissions( array( 'order_id' => $post->ID ) );
					$referral_history = get_post_meta( $post->ID, '_yith_wcaf_referral_history', true );
				}
			}

			include( YITH_WCAF_DIR . 'templates/admin/referral-commissions-metabox.php' );
		}
		...
		public static function get_instance(){
			if( class_exists( 'YITH_WCAF_Commission_Handler_Premium' ) ) {
				if ( is_null( YITH_WCAF_Commission_Handler_Premium::$instance ) ) {
					YITH_WCAF_Commission_Handler_Premium::$instance = new YITH_WCAF_Commission_Handler_Premium;
				}

				return YITH_WCAF_Commission_Handler_Premium::$instance;
			}
			else{
