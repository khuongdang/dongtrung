...
if ( ! class_exists( 'YITH_WCAF_Affiliate_Handler' ) ) {
	...
	class YITH_WCAF_Affiliate_Handler {
		...
		public function __construct() {
			...
			add_action( 'woocommerce_created_customer', array( $this, 'register_affiliate' ), 10, 1 );
		...
		public function print_affiliate_fields(){
			...
			if( 'any' == $enabled_form && in_array( 'woocommerce_register_form_start', $wp_current_filter) && 'yes' == $show_name_field ):
				...
				<p class="form-row form-row-wide">
					<label for="first_name"><?php _e( 'First name', 'yith-wcaf' ); ?></label>
		...
		public function add_screen_option() {
			if ( 'yith-plugins_page_yith_wcaf_panel' == get_current_screen()->id && isset( $_GET['tab'] ) && $_GET['tab'] == 'affiliates' ) {
				add_screen_option( 'per_page', array( 'label' => __( 'Affiliates', 'yith-wcaf' ), 'default' => 20, 'option' => 'edit_affiliates_per_page' ) );
			...
			if( isset( $_GET['tab'] ) && $_GET['tab'] == 'affiliates' ) {
				$columns = array_merge(
					...
					array(
						...
						'conversion' => __( 'Conversion', 'yith-wcaf' ),
		...
		public function render_affiliate_extra_fields( $user ) {
			...
			?>
			<hr />
			<h3><?php _e( 'Affiliate details', 'yith-wcaf' )?></h3>
			<table class="form-table">
				<tr>
					<th><label for="affiliate"><?php _e( 'Affiliate', 'yith-wcaf' )?></label></th>
					<td>
						<input type="checkbox" name="affiliate" id="affiliate" value="1" <?php checked( $is_affiliate, true ) ?> />
						<span class="description"><?php _e( 'Check if this user is an affiliate', 'yith-wcaf' ) ?></span>
					</td>
				</tr>
				<tr>
					<th><label for="enabled"><?php _e( 'Enabled', 'yith-wcaf' )?></label></th>
					<td>
				...
				<tr>
					<th><label for="token"><?php _e( 'Token', 'yith-wcaf' )?></label></th>
					<td>
						<input type="text" name="token" id="token" value="<?php echo esc_attr( $token ) ?>" />
						<span class="description"><?php _e( 'Token for the user (default to user ID)', 'yith-wcaf' ) ?></span>
					</td>
				</tr>
				<tr>
					<th><label for="rate"><?php _e( 'Rate', 'yith-wcaf' )?></label></th>
					<td>
				...
				<tr>
					<th><label for="payment_email"><?php _e( 'Payment email', 'yith-wcaf' )?></label></th>
					<td>
						<input type="email" name="payment_email" id="rate" value="<?php echo esc_attr( $payment_email ) ?>" />
						<span class="description"><?php _e( 'Address email where affiliate wants to receive PayPal payments', 'yith-wcaf' ) ?></span>
					</td>
				</tr>
