...
if ( ! class_exists( 'YITH_WCAF_Click_Handler' ) ) {
	...
	class YITH_WCAF_Click_Handler {
		...
		public function __construct() {
			...
			add_action( 'woocommerce_order_status_pending-payment', array( $this, 'unregister_conversion' ), 10, 1 );
		...
		public function register_hit(){
			...
			if( $affiliate && $token !== false && isset( $_REQUEST[ $token_name ] ) ){
				...
				$requester_link = $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
		...
		public function register_conversion( $order_id ) {
			...
			$row_to_change = get_post_meta( $order_id, '_yith_wcaf_click_id', true );
			$already_registered = get_post_meta( $order_id, '_yith_wcaf_conversion_registered', true );
			...
			if( $res ) {
				...
				// update order meta
				update_post_meta( $order_id, '_yith_wcaf_conversion_registered', true );
			}
		...
		public function unregister_conversion( $order_id  ) {
			...
			if( $res ) {
				...
				// update order meta
				update_post_meta( $order_id, '_yith_wcaf_conversion_registered', false );
			}
		...
		public function get_last_hit(){
			...
			$rows = $this->get_hits( array( 'user_id' => $user_id, 'IP' => $requester_ip, 'limit' => 1, 'order' => 'DESC' ) );
		...
		public static function get_instance(){
			if( class_exists( 'YITH_WCAF_Click_Handler_Premium' ) ) {
				if ( is_null( YITH_WCAF_Click_Handler_Premium::$instance ) ) {
					YITH_WCAF_Click_Handler_Premium::$instance = new YITH_WCAF_Click_Handler_Premium;
				}

				return YITH_WCAF_Click_Handler_Premium::$instance;
			}
			else{
