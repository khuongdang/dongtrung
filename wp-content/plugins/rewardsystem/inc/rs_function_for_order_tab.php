...
class RSFunctionForOrder {
    ...
    public function display_line_total1($line_total1, $id, $item) {
        ...
        $linetotal = array();
        $updatedvalue = array();
        if ($rewardgateway == 'reward_gateway' || $rewardgateway == '') {
            ...
            foreach ($order->get_items()as $item) {
                ...
                if (RSFunctionForCart::check_display_price_type($productid) == '2') {
                    ...
                    if ($enable[$productid] != '') {
                        ...
                        $newvalue = $linetotal / get_option('rs_redeem_point_value');
    ...
    public function display_total_point_price($order1, $orderids) {
        ...
        if ($rewardgateway == 'reward_gateway' || $rewardgateway == '') {
            ...
            foreach ($order->get_items()as $item) {
                ...
                $newvalue1 = $shipping_total / get_option('rs_redeem_point_value');
                $updatedvalue1 = $newvalue1 * get_option('rs_redeem_point');
